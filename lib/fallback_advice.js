...
export function getFallbackOutfit(weatherData, profileKey) {
  ...
  if (profile && profile !== 'звичайний') {
    if (profileKey === 'office') explanation += ' Раджу щось зручне для приміщення — у офісі може бути тепліше.';
    else if (profileKey === 'walk') explanation += ' Підійде універсальний одяг для прогулянки.';
    else if (profileKey === 'run') explanation += ' На пробіжку краще вдягнутися трохи легше — від руху буде тепліше.';
    else if (profileKey === 'bike') explanation += ' На велосипеді вітер охолоджує — краще трохи легше ніж на пішки, та щось світле для видимості.';
    else if (profileKey === 'baby') explanation += ' З дитиною варто мати запасний шар або плед.';
    else if (profileKey === 'car') explanation += ' За кермом достатньо зручного одягу.';
    else explanation += ` Враховую профіль: ${profile}.`;
  }
  ...
  if (temp != null) {
    if (temp < -5) {
      ...
      if (profileKey === 'walk') bullets.push('На прогулянку — рукавички та тепле взуття обовʼязково.');
      if (profileKey === 'run') bullets.push('На пробіжку — багатошаровість, дихаючий верхній шар.');
      if (profileKey === 'car') bullets.push('За кермом — зручний одяг, прогрійте салон перед поїздкою.');
    } else if (temp < 5) {
      ...
      if (profileKey === 'walk') bullets.push('На прогулянку — шапка та шарф не завадять.');
      if (profileKey === 'run') bullets.push('На пробіжку — на шар легше ніж на прогулянку.');
      if (profileKey === 'car') bullets.push('За кермом — у салоні тепліше, зручний одяг.');
    } else if (temp < 15) {
      bullets.push('🧥 Легка куртка або светр');
      bullets.push('👕 Можна без шапки; мати щось тепліше під рукою');
      bullets.push('👟 Закрите взуття (не сандалі — ще прохолодно)');
      if (profileKey === 'office') bullets.push('Для офісу — верх, який легко зняти в приміщенні.');
      if (profileKey === 'walk') bullets.push('На прогулянку — зручне взуття, можна шарувати за бажанням.');
      if (profileKey === 'run') bullets.push('На пробіжку — на шар легше, дихаючий одяг.');
      if (profileKey === 'car') bullets.push('За кермом — зручний верх, у салоні зазвичай тепліше.');
      if (profileKey === 'bike') bullets.push('На велосипеді — трохи легше ніж на прогулянку, вітер при їзді охолоджує.');
    } else if (temp < 25) {
      bullets.push('👕 Легкий одяг, довгий рукав вранці/ввечері');
      bullets.push('👟 Зручне взуття (відкрите — лише коли справді тепло)');
      bullets.push('Сонцезахист у сонячний день.');
      if (profileKey === 'office') bullets.push('Для офісу — легкий верх, в приміщенні може бути тепло.');
      if (profileKey === 'walk') bullets.push('На прогулянку — зручне взуття, легкий шар на випадок вітру.');
      if (profileKey === 'run') bullets.push('На пробіжку — легкий дихаючий одяг.');
      if (profileKey === 'car') bullets.push('За кермом — легкий одяг, кондиціонер може охолодити.');
      if (profileKey === 'bike') bullets.push('На велосипеді — легко, вітер при їзді охолоджує.');
    } else {
      ...
      if (profileKey === 'walk') bullets.push('На прогулянку — головний убір та вода.');
      if (profileKey === 'run') bullets.push('На пробіжку — мінімум одягу, обовʼязково вода.');
      if (profileKey === 'car') bullets.push('За кермом — легкий одяг, кондиціонер у салоні.');
    }
  }
  ...
  if (profileKey === 'bike') bullets.push('На велосипеді: світлі або яскраві елементи для видимості.');
