...
/** WMO code (Open-Meteo) → короткий опис українською (орієнтовно). */
function openMeteoWmoToUa(code) {
  const c = Number(code);
  if (Number.isNaN(c)) return 'н/д';
  if (c === 0) return 'Ясно';
...
/**
 * До 16 днів без ключа (Open-Meteo). Лише для дат, яких немає у WAPI/OW — орієнтовна модель.
 * https://open-meteo.com/ — умови використання та атрибуція.
 */
async function tryGetForecastDaysOpenMeteo(lat, lon) {
  try {
    ...
    if (!res.ok) {
      console.warn('Open-Meteo forecast:', res.status);
      return null;
    }
    const data = await res.json();
    const daily = data.daily;
    ...
    for (let i = 0; i < times.length; i++) {
      ...
      days.push({
        ...
        condition: openMeteoWmoToUa(wcode),
        chanceOfRain: pop != null ? Math.round(pop) : 0,
        windMaxMs: null,
        tempMorning: null,
        tempNoon: null,
        tempEvening: null,
        forecastApproximate: true,
      });
    }
    return { timezoneOffsetSeconds: data.utc_offset_seconds ?? 0, days };
  } catch (e) {
    console.warn('Open-Meteo forecast:', e.message || e);
    return null;
  }
}

/** Додати дні з Open-Meteo лише для дат, яких ще немає (не перекриває WAPI/OW). */
function mergeOpenMeteoGaps(primaryResult, omResult) {
  if (!primaryResult?.days?.length || !omResult?.days?.length) return primaryResult;
  const map = new Map();
  for (const d of primaryResult.days) {
    if (d?.date) map.set(d.date, d);
  }
  for (const d of omResult.days) {
    if (d?.date && !map.has(d.date)) map.set(d.date, d);
  }
  const days = [...map.keys()].sort().map((k) => map.get(k));
  const prOff = primaryResult.timezoneOffsetSeconds ?? 0;
  const omOff = omResult.timezoneOffsetSeconds ?? 0;
  /** Якщо основний шар дав 0 (рідко), беремо зсув з Open-Meteo — інакше «сьогодні» зміщується. */
  const timezoneOffsetSeconds = prOff !== 0 ? prOff : omOff;
  return {
...
export async function getForecastDays(lat, lon) {
  ...
  const om = await tryGetForecastDaysOpenMeteo(lat, lon);
  if (om?.days?.length) {
    result = mergeOpenMeteoGaps(result, om);
  }
