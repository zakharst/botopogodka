...
export async function getForecastDays(lat, lon) {
  ...
  if (WEATHERAPI_KEY) {
    ...
    if (!res.ok) {
      const t = await res.text();
      throw new Error(`WeatherAPI forecast: ${res.status} ${t}`);
    }
    const data = await res.json();
    ...
    result = { timezoneOffsetSeconds: offsetSeconds, days };
  } else if (API_KEY) {
    const res = await fetch(`${ONE_CALL_URL}?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric&lang=ua`);
    if (!res.ok) {
      const fallback = await getForecastDaysOpenWeather25(lat, lon);
      if (fallback && fallback.days.length) {
        result = fallback;
    ...
    result = { timezoneOffsetSeconds, days };
  } else {
