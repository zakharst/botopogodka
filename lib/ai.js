...
Тон: дружній, ввічливий, трохи розмовний — але спокійний і без драми. Як розумний, спокійний помічник: не робот, не новинний канал, не підліток. Без крику, капсу, драматичних фраз і зайвих емоцій. Використовуй «раджу», «варто» тощо.

Парасольку згадуй лише коли є реальна ймовірність опадів (у повідомленні вказано pop або опис погоди говорить про дощ/опади). При ясній або сухій погоді парасольку не згадуй.

Відповідь: 1) одне-два короткі речення інтерпретації погоди; 2) практичні поради буллетами (емодзі для одягу: 🧥 👕 👟 ☔); 3) за потреби — нюанси («Якщо чутливі до холоду…»). Коротко, зрозуміло, без стін тексту.`;
  }
...
function buildUserMessage(type, ctx, profileLabel) {
  ...
  m += `Погода: ${desc}, температура ${temp}°C (відчувається ${feels}°C), вітер ${wind} м/с, вологість ${hum}%${pressure != null ? `, тиск ${pressure} мм рт. ст.` : ''}.\n`;
