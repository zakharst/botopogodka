...
Не давай медичних чи лікувальних порад. Тільки рекомендації: що вдягнути, чи брати парасольку, чи вітер сильний тощо.`;
  if (type === 'outfit') {
    return `${base}\n\nВідповідь має містити: 1) короткий список рекомендацій буллетами (3-5 пунктів); 2) одне-два речення пояснення. Без зайвих вступів.`;
  }
...
function buildUserMessage(type, ctx, profileLabel) {
  ...
  m += `Погода: ${desc}, температура ${temp}°C (відчувається ${feels}°C), вітер ${wind} м/с, вологість ${hum}%.\n`;
