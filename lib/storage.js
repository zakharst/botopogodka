...
/**
 * Один раз забронювати ключ (NX + TTL). Повертає true, якщо бронь наша.
 * Для розсилок, щоб не дублювати при повторних викликах cron.
 */
export async function tryClaimBroadcastLock(key, ttlSeconds = 172800) {
  const r = await redis('SET', key, '1', 'NX', 'EX', String(ttlSeconds));
  return r === 'OK';
}
