...
/** Заголовок блоку з алгоритмічним висновком (після тексту AI). */
export const WARM_WINDOW_FACTS_HEADING = '<b>Коротко з прогнозу</b>';
...
/** Окремий рядок температури (як у блоці «По днях»). */
function formatWarmDayTempLine(day) {
  if (day.tempMin != null && day.tempMax != null) {
    const a = `${day.tempMin >= 0 ? '+' : ''}${day.tempMin}°`;
    const b = `${day.tempMax >= 0 ? '+' : ''}${day.tempMax}°`;
    return `Від ${a} до ${b}`;
  }
  if (day.tempMax != null) return `До ${day.tempMax >= 0 ? '+' : ''}${day.tempMax}°`;
  if (day.tempMin != null) return `Від ${day.tempMin >= 0 ? '+' : ''}${day.tempMin}°`;
  return 'Температура: н/д';
}

function warmDayBlock(day) {
  return `<b>${captionForDate(day.date)}</b>\n${formatWarmDayTempLine(day)}`;
}
...
/**
 * Рядок про горизонт — одразу під заголовком повідомлення (не «посеред» тексту).
 * `null`, якщо днів немає.
 */
export function formatWarmWindowScopeHtml(forecastData) {
  const horizon = horizonDaysOrFallback(forecastData, 16);
  if (!horizon.length) return null;
  const n = horizon.length;
  const first = captionForDate(horizon[0].date);
  const last = captionForDate(horizon[horizon.length - 1].date);
  return `<i>Беремо найближчі ${n} ${daysWordUk(n)} від сьогодні: від <b>${first}</b> до <b>${last}</b> (час у вашому місті).</i>`;
}

/**
 * Висновок по днях (HTML), без заголовка «Коли потепліє» й без рядка про горизонт.
 */
export function formatWarmWindowBodyHtml(forecastData) {
  ...
  if (stableDays && stableDays.length >= C.STABLE_WINDOW_MIN) {
    ...
    const dayBlocks = stableDays.map((d) => warmDayBlock(d)).join('\n\n');
    const rain = rainLineForDays(stableDays);

    let intro;
    let note;
    if (stableDays.length >= C.STABLE_WINDOW_MAX) {
      intro = `Схоже, стабільніший приємніший період — з <b>${a}</b> по <b>${b}</b> (${stableDays.length} ${daysWordUk(stableDays.length)} поспіль):`;
    } else {
      intro =
        stableDays.length === 2
          ? 'Найближче тепліше «вікно» — два дні поспіль:'
          : 'Найближче тепліше «вікно» — кілька днів поспіль:';
      note = 'Довкола в прогнозі може бути прохолодніше — це короткий відрізок.';
    }
    body = [intro, dayBlocks, note, rain].filter(Boolean).join('\n\n');
  } else {
    ...
    if (spikeIdx >= 0) {
      ...
      const tail = laterStable
        ? `Стабільніше тепло далі — приблизно <b>${captionForDate(laterStable[0].date)}</b> — <b>${captionForDate(laterStable[laterStable.length - 1].date)}</b>.`
        : 'Далі в прогнозі знову прохолодніше.';
      body = [
        'Є один день, що виділяється теплішим на тлі інших:',
        warmDayBlock(d),
        tail,
        rainLineForDays([d]),
      ].join('\n\n');
      ...
      const intro =
        pick.score >= C.DAY_SCORE_WEAK
          ? 'Найменш «холодний» день у цьому відрізку:'
          : 'Найтепліший день у прогнозі:';
      body = [intro, warmDayBlock(d), rainLineForDays([d])].join('\n\n');
    }
  }
...
export function formatWarmWindowMessage(forecastData) {
  ...
  const scope = formatWarmWindowScopeHtml(forecastData);
  return `${WARM_WINDOW_HEADER}${scope}\n\n${body}`;
}
