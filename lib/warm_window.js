...
import { sliceForecastFromToday, escapeHtml } from './format.js';

export const WARM_WINDOW_CONFIG = {
  /** Мало днів — чесно кажемо, що це короткий горизонт */
  MIN_TOTAL_DAYS_SHORT: 5,
  /** Нижче цього — додаткова примітка про ненадійність */
  MIN_TOTAL_DAYS_MEDIUM: 10,
  STABLE_WINDOW_MIN: 2,
  STABLE_WINDOW_MAX: 3,
...
function rainLineForDays(days) {
  ...
  if (mx >= 55) return `Опади: до ${Math.round(mx)}% за день у моделі.`;
  if (mx >= 25) return `Опади: помірна ймовірність (до ${Math.round(mx)}%).`;
  return 'Опади: без суттєвих у цьому фрагменті прогнозу.';
}
...
/**
 * Текст відповіді (HTML як у решті бота).
 */
export function formatWarmWindowMessage(forecastData, cityDisplay) {
  const C = WARM_WINDOW_CONFIG;
  const horizon = sliceForecastFromToday(forecastData, 16);

  if (horizon.length === 0) {
    return 'Не вдалося взяти жодного дня прогнозу — спробуйте пізніше.';
  }

  const scored = horizon.map((day) => ({ score: scoreForecastDayComfort(day), day }));
  const approxN = horizon.filter((d) => d.forecastApproximate).length;

  const meta = [];
  if (horizon.length < C.MIN_TOTAL_DAYS_SHORT) {
    meta.push(
      `<i>Короткий горизонт (${horizon.length} дн.) — висновок дуже орієнтовний.</i>`,
    );
  } else if (horizon.length < C.MIN_TOTAL_DAYS_MEDIUM) {
    meta.push(`<i>У прогнозі ${horizon.length} днів — далі невизначеність зазвичай більша.</i>`);
  }
  if (approxN > 0) {
    meta.push('<i>Частина дат з моделі Open-Meteo (довгострок грубший).</i>');
  }
  ...
  const cityLine = cityDisplay
    ? `Прогноз для: ${escapeHtml(cityDisplay)}\n\n`
    : '';
  ...
  if (stableDays && stableDays.length >= C.STABLE_WINDOW_MIN) {
    ...
    if (spikeIdx >= 0) {
      ...
      const tail = laterStable
        ? ` Стабільніше тепло ближче до <b>${captionForDate(laterStable[0].date)}</b> — <b>${captionForDate(laterStable[laterStable.length - 1].date)}</b>.`
        : ' Стабільного тепла далі в горизонті поки не видно.';
      body = `Є один відносно тепліший день — <b>${cap}</b> (${tempSpan(d) || '—'}).${tail} ${rainLineForDays([d])}`;
    } else {
      const weak = scored.find((x) => x.score >= C.DAY_SCORE_WEAK);
      if (weak) {
        body = `Чіткого «нарешті тепло» в доступних ${horizon.length} днях не видно. Найменш «холодний» сигнал — <b>${captionForDate(weak.day.date)}</b> (${tempSpan(weak.day) || '—'}), але це слабко.`;
      } else {
        body = `У цьому горизонті немає помітного стабільного потепління. Можливо, варто зайти пізніше — або погода справді залишиться прохолодною за моделлю.`;
      }
    }
  }

  const header = '🌡 <b>Коли нарешті потепліє?</b> <i>(експериментально)</i>\n\n';
  const disclaimer =
    '\n\n<i>Це евристика по денних даних API, не точний прогноз. Довгі інтервали завжди менш надійні.</i>';

  return header + cityLine + body + (meta.length ? `\n\n${meta.join('\n')}` : '') + disclaimer;
}
