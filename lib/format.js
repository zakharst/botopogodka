...
/** День тижня 0–6 (0 = неділя) для дати YYYY-MM-DD у заданому часовому поясі. */
function dayOfWeekFromDate(dateStr, offsetSeconds) {
  const [y, m, d] = dateStr.split('-').map(Number);
  const localMidnightMs = Date.UTC(y, m - 1, d) - (offsetSeconds || 0) * 1000;
  return new Date(localMidnightMs).getUTCDay();
}

/** Форматувати один день прогнозу: "Понеділок, 10 березня · 5°…12°, дощ · 80% опади" */
function formatForecastDayLine(day, offsetSeconds) {
  const dow = dayOfWeekFromDate(day.date, offsetSeconds);
  ...
  return `${dayName}, ${dateStr}\n${tempStr}, ${day.condition}${rain}`;
}

/** Дружній розгорнутий блок на один день (для вихідних): температура по часах, дощ, вітер. */
function formatForecastWeekendDayBlock(day, offsetSeconds) {
  const dow = dayOfWeekFromDate(day.date, offsetSeconds);
  const dayName = DAY_NAMES_UA[dow];
  const [y, month, date] = day.date.split('-').map(Number);
  const monthsUA = ['', 'січня', 'лютого', 'березня', 'квітня', 'травня', 'червня', 'липня', 'серпня', 'вересня', 'жовтня', 'листопада', 'грудня'];
  ...
  if (day.tempMorning != null || day.tempNoon != null || day.tempEvening != null) {
    ...
    parts.push(`Вранці ${m}, обід ${n}, ввечері ${e}`);
  }
  if (day.chanceOfRain > 0) {
    parts.push(`Дощ: ймовірність ${day.chanceOfRain}%`);
  } else {
    parts.push('Опадів не очікується');
  }
  if (day.windMaxMs != null && day.windMaxMs > 0) {
    parts.push(`Вітер до ${day.windMaxMs} м/с`);
  }
...
export function formatForecastWeekend(forecastData, cityDisplay) {
  ...
  let foundSun = false;
  for (const d of days) {
    const dow = dayOfWeekFromDate(d.date, timezoneOffsetSeconds);
    if (dow === 6 && !foundSat) {
      weekend.push(d);
      foundSat = true;
  ...
  const body = weekend.map((d) => formatForecastWeekendDayBlock(d, timezoneOffsetSeconds)).join('\n\n');
  return header + body;
}
...
export function formatForecastWeek(forecastData, cityDisplay) {
  ...
  const body = week.map((d) => formatForecastDayLine(d, timezoneOffsetSeconds)).join('\n\n');
  return header + body;
}
...
export function formatForecast14Days(forecastData, cityDisplay) {
  ...
  const body = days.map((d) => formatForecastDayLine(d, timezoneOffsetSeconds)).join('\n\n');
  return header + body;
}
