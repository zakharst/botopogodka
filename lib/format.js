...
export function formatWeather(weatherData, cityDisplay, localTime) {
  ...
  const city = escapeHtml(cityDisplay || 'Місто');
  ...
  const tempRange = tempMin != null && tempMax != null ? `${tempMin}°…${tempMax}°` : (c.temp != null ? `+${Math.round(c.temp)}°` : '—');
  ...
  const alertRain = pop > 30 ? `Ймовірний дощ після 17:00.` : '';

  let text = `${city} сьогодні\n`;
  text += `${tempRange}, ${desc}.\n`;
  if (alertRain) text += `${alertRain}\n`;
  text += '\n';
  ...
  text += 'Якщо хочете поради щодо одягу — просто напишіть «що вдягнути».';
  return text;
}
