...
function openMeteoAttributionFooter(daysShown) {
  if (!daysShown?.some((d) => d.forecastApproximate)) return '';
  return '\n\n<i>Деякі дні доповнені орієнтовно за моделлю Open-Meteo (open-meteo.com).</i>';
}
...
export function formatForecastWeek(forecastData, cityDisplay) {
  ...
  const week = rollingForecastDaysFromToday(days, timezoneOffsetSeconds, 7);
  if (week.length === 0) return 'Немає даних для прогнозу на тиждень.';
  const cityInLocative = cityDisplay ? cityDisplayToLocative(cityDisplay) : null;
  const header = cityInLocative
    ? `📆 Наступні 7 днів ${cityInLocative}\n\n`
    : (cityDisplay ? `📆 Наступні 7 днів (${cityDisplay})\n\n` : '📆 Наступні 7 днів\n\n');
  const body = week.map((d) => formatForecastDayLine(d)).join('\n\n');
  return header + body + partialForecastFooter(week.length, 7) + openMeteoAttributionFooter(week);
}

/** Прогноз на 14 днів від сьогодні (місцевий час міста). */
export function formatForecast14Days(forecastData, cityDisplay) {
  const { days = [], timezoneOffsetSeconds = 0 } = forecastData;
  const fortnight = rollingForecastDaysFromToday(days, timezoneOffsetSeconds, 14);
  if (fortnight.length === 0) return 'Немає даних для прогнозу.';
  const cityInLocative = cityDisplay ? cityDisplayToLocative(cityDisplay) : null;
  const header = cityInLocative
    ? `📆 Наступні 14 днів ${cityInLocative}\n\n`
    : (cityDisplay ? `📆 Наступні 14 днів (${cityDisplay})\n\n` : '📆 Наступні 14 днів\n\n');
  const body = fortnight.map((d) => formatForecastDayLine(d)).join('\n\n');
  return header + body + partialForecastFooter(fortnight.length, 14) + openMeteoAttributionFooter(fortnight);
}
